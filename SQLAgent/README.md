...
A comprehensive educational repository demonstrating the progression from basic SQL agents to production-ready, secure analytics systems using LangChain and Google Gemini (2.5 Flash). Learn by example how to build safe, powerful SQL agents for business intelligence and analytics using Gemini.
...
- Python 3.8+
- Google Gemini API key (set as GOOGLE_API_KEY in your .env)
...
cp .env.example .env
# Edit .env and add your Gemini API key: GOOGLE_API_KEY=your-gemini-api-key-here
...
python scripts/04_complex_queries.py    # Advanced analytics chatbot (CLI)
```
...
- 💬 **Multi-turn CLI chatbot** for iterative data exploration
...
- Interactive CLI chatbot for analytics and SQL Q&A
...
**🎯 Remember**: The goal is to understand the progression from dangerous to secure SQL agent implementations. Start with the basics, understand the risks, then build robust, production-ready solutions. Now powered by Google Gemini 2.5 Flash and a CLI chatbot for interactive analytics.
